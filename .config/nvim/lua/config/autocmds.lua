...
autocmd("LspAttach", {
  ...
  callback = function(ev)
    ...
    -- NOTE: Populate workspace diagnostics
    workspace_diagnostics.populate_workspace_diagnostics(client, vim.api.nvim_get_current_buf())
  end,
...
autocmd({ "ColorScheme", "WinNew", "BufWinEnter", "FileType" }, {
  callback = function()
    pcall(function()
      require("config.utils").transparency.apply_if_enabled()
    end)
  end,
  group = general,
  desc = "Re-apply manual transparency for new windows and UIs",
})
